...
full Bayesian statistical inference with MCMC sampling in addition to other inference
algorithms.
Stan’s math library provides differentiable probability functions & linear algebra (C++ autodiff). Additional R packages provide expression-based linear modeling, posterior visualization, Bayesian cross-validation, and much more.

Stan interfaces with the most popular data analysis languages (R, Python, Julia) as well as the command line. It runs on all major platforms (Linux, Mac, Windows). 
Stan is freedom-respecting, open-source software (new BSD core, some interfaces GPLv3).  Stan is associated with [NumFOCUS](https://numfocus.org/), a nonprofit supporting open code and reproducible science, through which you can help support Stan.
...
- **bayesim**: Simulations from Bayesian models [[GitHub](https://github.com/sims1253/bayesim)]
