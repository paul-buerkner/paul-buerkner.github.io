...
## grammo: A grammar of probabilistic models {#grammo}

This project aims to design and implement grammo: a novel mid-level grammar 
for expressing Bayesian statistical models. Grammo will provide substantially more flexibility, 
reusability and transparency than state-of-the-art high-level statistical packages while staying much more
concise and accessible than low-level probabilistic programming languages. Beyond theoretical
analysis of its properties, the usability of grammo will be evaluated with human users.
We will then develop new methods for automatic and computer-assisted analysis, manipulation
and debugging of probabilistic programs, using grammo as a testbed. Further, grammo will
allow us to research several classes of complex models that are currently rarely used due to
difficulties in their specification. This includes various forms of joint models combining multiple
data-generating mechanisms in a single model with shared parameters as well as time-to-event
models and non-standard structural equation models. We will then develop workflows and
supporting tools for those models including prior choice, model validation and selection.
...
[Czech Science Foundation (GACR)](https://gacr.cz/en/){.btn .btn-outline-primary .btn role="button" .btn-page-header .btn-xs}
...
#| echo: false
#| output: asis
# render_publications(project = "grammo", author = c("Habermann, D."))
```

<!-- -->

## Deep Bayes in Biomedicine {#deep-bayes-biomed}

Progress in biomedical research is limited by three challenges: (1) the systems we study, like
the immune system or the brain, are complex and variable; (2) experiments often have small
sample sizes because of ethical or cost constraints; and (3) the effects we look for are often
small, comparable to normal biological variation.

Bayesian statistics is a powerful approach for tackling such problems, allowing inclusion
of prior knowledge and principled uncertainty quantification. This makes conclusions more
reliable and reproducible. However, Bayesian inference is computationally far more expensive
than conventional data analyses, limiting its practicality for modern biomedical experiments.

Amortized Bayesian Inference (ABI), an approach based on deep learning, removes this
barrier. By “learning” in advance how to perform Bayesian inference, ABI can analyze new
datasets almost instantly. It matches or exceeds the speed of conventional methods while
retaining the full advantages of Bayesian inference. This project applies ABI to challenging
biomedical workflows in immunology and neuroscience, where current tools fall short. By
combining expertise from biomedicine, deep learning, and statistics, we make ABI easier to
use and adapt, enabling researchers to gain biomedical insights faster and more reliably.
...
Project Members: 
[Yiming Zang](../people#yiming-zang){.btn .btn-outline-primary .btn role="button" .btn-page-header .btn-xs}

Funders: 
[MERCUR Foundation](https://www.uaruhr.de/en/research/research-support/mercur-project-funding/){.btn .btn-outline-primary .btn role="button" .btn-page-header .btn-xs}
...
#| echo: false
#| output: asis
# render_publications(project = "deep-bayes-biomed", author = c("Zang, Y."))
```
...
#| echo: false
#| output: asis
render_publications(project = "abi-logistics", author = "Jedhoff, S.")
```

<!-- -->

## Semi-Supervised Learning for Robust Amortized Bayesian Inference {#semi-supervised-abi}

![](../images/self_consistency_workflow.png){width=80% fig-align="left" fig-alt="Results obtained from semi-supervised ABI training."}
...
#| echo: false
#| output: asis
render_publications(project = "semi-supervised-abi", author = "Mishra, A.")
```

<!-- -->

## BayesFlow: Simulation Intelligence with Deep Learning {#bayesflow-sim-intelligence}
...
[Hans Olischläger](../people#Hans-olischlaeger){.btn .btn-outline-primary .btn role="button" .btn-page-header .btn-xs}
...
#| echo: false
#| output: asis
render_publications(project = "bayesflow-sim-intelligence", author = c("Kühmichel, L.", "Olischläger, H."))
```
...
#| echo: false
#| output: asis
render_publications(project = "abi-applications", author = "Kucharský, Š.")
```
...
#| echo: false
#| output: asis
render_publications(project = "amortized-mlms", author = "Habermann, D.")
```
...
Project Members: [Florence Bockting](https://florence-bockting.github.io){.btn .btn-outline-primary .btn role="button" .btn-page-header .btn-xs}
...
#| echo: false
#| output: asis
render_publications(project = "sbpriors", author = "Bockting, F.")
```
...
Project Members: [Luna Fazio](https://lunafazio.github.io){.btn .btn-outline-primary .btn role="button" .btn-page-header .btn-xs}
...
#| echo: false
#| output: asis
render_publications(project = "bdlvms", author = "Fazio, L.")
```
...
Project Members: [Soham Mukherjee](https://www.linkedin.com/in/soham-mukherjee-33a397146){.btn .btn-outline-primary .btn role="button" .btn-page-header .btn-xs}
...
#| echo: false
#| output: asis
render_publications(project = "scrna-models", author = "Mukherjee, S.")
```
...
#| echo: false
#| output: asis
render_publications(project = "uncertainty-surrogate-models", author = "Reiser, P.")
```
...
::: {.callout-note icon=false collapse="true"}
## Meta-Uncertainty in Bayesian Model Comparison {#mu-bmc}
...
#| echo: false
#| output: asis
render_publications(project = "mu-bmc", author = "Schmitt, M.")
```
...
Project Members: [Javier Aguilar](https://jear2412.github.io){.btn .btn-outline-primary .btn role="button" .btn-page-header .btn-xs}
...
#| echo: false
#| output: asis
render_publications(project = "joint-priors-mlms", author = "Aguilar, J. E.")
```
...
#| echo: false
#| output: asis
render_publications(project = "ml4bmb", author = "Scholz, M.")
```
